...
import { ICert } from "@/app/interfaces/cert";

export function Card({ cert: cert }: { cert: ICert }) {
    return (
        <div className="group relative my-10 grid grid-cols-8 gap-4 transition-all sm:items-center sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
            ...
            <div className="z-10 col-span-6">
                ...
                <h3 className="-mt-1">
                    <a className="inline-flex items-baseline font-medium leading-tight text-slate-200 hover:text-sky-300 focus-visible:text-sky-300  group/link text-base" href={`${cert.url}`} target="_blank" rel="noreferrer noopener" aria-label="Integrating Algolia Search with WordPress Multisite (opens in a new tab)">
                        <span className="absolute -inset-x-4 -inset-y-2.5 hidden rounded md:-inset-x-6 md:-inset-y-4 lg:block"></span>
                        <span className="text-lg">{cert.name} · <span className="inline-flex">{cert.provider}
                            <Image
